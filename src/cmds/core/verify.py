...
class VerifyCog(commands.Cog):
    ...
    @slash_command(
        guild_ids=settings.guild_ids, description="Verify your HTB Certifications!"
    )
    @cooldown(1, 60, commands.BucketType.user)
    async def verifycertification(
        self, ctx: ApplicationContext, certid: str, fullname: str
    ) -> Interaction | WebhookMessage:
        """Verify HTB Academy certificate."""
        if not certid or not fullname:
            await ctx.respond("You must supply a cert id!")
            return
        if not certid.startswith("HTBCERT-"):
            await ctx.respond("CertID must start with HTBCERT-")
            return
        cert = await process_certification(certid, fullname)
        if cert:
            to_add = settings.get_cert(cert)
            await ctx.author.add_roles(to_add)
            await ctx.respond(f"Added {cert}!")
        else:
            await ctx.respond("Unable to find certification with provided details")
    ...
    async def verify(self, ctx: ApplicationContext) -> Interaction | WebhookMessage:
        ...
        return await ctx.respond(
            "Please check your DM for instructions.", ephemeral=True
        )
