...
class BanCog(commands.Cog):
    ...
    async def ban(
        ...
        response = await ban_member(self.bot, ctx.guild, member, "500w", reason, evidence, ctx.user, needs_approval=False)
    ...
    async def tempban(
        ...
        if not member:
            return await ctx.respond(f"User {user} not found.")
        await self.add_evidence_note(member.id, reason, evidence, ctx.user.id)
        response = await ban_member(self.bot, ctx.guild, member, duration, reason, evidence, ctx.user, needs_approval=True)
        return await ctx.respond(response.message, delete_after=response.delete_after)
